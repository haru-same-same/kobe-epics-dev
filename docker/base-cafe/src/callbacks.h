...
#define __METHOD__  "callbackHandlerMonitor"
    ...
    if (it_handle != handle_index.end()) {
				std::cout << "MONITOR UPDATE " << std::endl;
        std::cout << "PV= " << (*it_handle).getPV() << " HANDLE= " << (*it_handle).getHandle() << std::endl;
...
#define __METHOD__  "handlerGet"
    ...
    if (it_handle != handle_index.end())  {
            ...
            handle_index.modify(it_handle, change_eventHandlerArgs   (args));
            	...
            	(*it_handle).getPVDataHolder(pvd);
            	pvd.print();
            	std::cout << "USER SUPPLIED CALLBACK: val/D//= " << pvd.getAsString(0) << std::endl;
